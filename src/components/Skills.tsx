        ...
        <div className="flex justify-center items-center mb-20">
          <div className="relative w-[650px] h-[650px] max-w-full">
            ...
            {skillCategories.map((category, index) => (
              <div 
                key={index} 
                className={`absolute ${category.position} glass-card p-5 shadow-lg w-[240px] reveal-item`}
                style={{ animationDelay: `${index * 150}ms` }}
              >
                <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${category.color} mb-3`}>
                  {category.title}
                </div>
                <div className="flex flex-col gap-2">
                  {category.skills.map((skill, skillIndex) => (
                    <div 
                      key={skillIndex} 
                      className="px-3 py-1 bg-white/80 rounded-md text-sm font-medium shadow-sm border border-gray-100"
                    >
                      {skill}
                    </div>
                  ))}
